...
## Task 05 — control plane built; build/verify handoff
...
`0.14.0` contract and pins `Maichess.PlatformProtos` `0.14.0`. The authoring shell
**cannot restore** that package (GitHub Packages returns 401 without credentials), so the
solution was **not** compiled or tested here. The user runs the .NET verify:

```bash
dotnet restore   # with GitHub Packages creds
...
Expected: 100% line/branch/method on non-excluded code. The Reqnroll `.feature.cs`
code-behind is generated by the Reqnroll MSBuild target during `dotnet test`.
